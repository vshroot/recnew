...
const pairSearchClear = ev.target.closest("[data-pair-search-clear]");
  if (pairSearchClear) {
    ...
    const input = resultsEl.querySelector(`[data-pair-search="${cssEscapeIdent(pairKey)}"]`);
    ...
    console.error(e);
    try { showBanner("error", "UI error", String(e && e.message ? e.message : e)); } catch (_) {}
  }
});
...
function showBanner(kind, title, text, actions = []) {
  ...
  for (const a of actions) {
    const btn = bannerHost.querySelector(`[data-banner-action="${cssEscapeIdent(a.id)}"]`);
...
function cssEscapeIdent(value) {
  ...
  try {
    if (typeof CSS !== "undefined" && CSS && typeof CSS.escape === "function") return cssEscapeIdent(s);
  } catch (_) {}
  // Fallback: conservative escaping for use inside attribute selectors
...
function csvCell(v) {
  ...
  function renderFiles() {
    ...
    for (const f of state.files) {
      const root = filesEl.querySelector(`[data-file-id="${cssEscapeIdent(f.id)}"]`);
      if (!root) continue;
...
function addFileWithPicker() {
  ...
  function reRenderPairTable(pairKey) {
    const pairBlock = resultsEl.querySelector(`[data-pair-key="${cssEscapeIdent(pairKey)}"]`);
    if (!pairBlock) return;
    const reportData = diffReports.get(pairKey);
    if (!reportData) return;
    const st = diffState.get(pairKey);
