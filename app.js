...
(() => {
  ...
  // reconciler_label_guard: if Add file is a <label for="filePicker">, the browser opens dialog natively.
  // We only need to support keyboard activation for accessibility.
  const __addFileEl = $("#addFileBtn");
  if (__addFileEl) {
    __addFileEl.addEventListener("keydown", (ev) => {
      if (ev.key === "Enter" || ev.key === " ") { ev.preventDefault(); __addFileEl.click(); }
    });
  }
  ...
  window.addEventListener("error", (e) => {
    try { showBanner("danger", "JS error", String(e.message || e.error || e)); } catch {}
  });
  window.addEventListener("unhandledrejection", (e) => {
    try { showBanner("danger", "Promise rejection", String(e.reason || e)); } catch {}
  });
const state = {
    files: [],
    primaryId: null,
...
function setProgress(text, pct) {
    ...
    if (progressTextEl) progressTextEl.textContent = String(text || "");
    else if (progressEl) {
      // Fallback: do NOT clobber the whole section; update a data-attr for debugging.
      progressEl.setAttribute("data-progress", String(text || ""));
    }
...
function addFileWithPicker() {
  ...
  // === Add file (GitHub Pages / Safari-safe) ===
  // index.html uses <label for="filePicker"> so the browser opens the dialog natively.
  // Here we only react to the selected file.
  (function bindTopFilePickerOnce(){
    const picker = document.querySelector('#filePicker');
    if (!picker) return;
    if (picker.__reconciler_bound) return;
    picker.__reconciler_bound = true;
    ...
    // Keyboard accessibility for the "Add file" label (Enter/Space).
...
function getGlobalSettings() {
    // Be defensive: missing controls should not crash the whole app.
    const elScale = $("#amountScale");
    const elTol   = $("#amountTolerance");
    const elLimit = $("#reportLimit");
    const elMap   = $("#statusMappings");

    // Fallbacks: use the visible "Tolerance" as a decimal tolerance if hidden inputs aren't present.
    const visibleTol = Number($("#tolerance")?.value);

    const amountScale = Number(elScale?.value);
    // amountTolerance is in "scaled units" (integer). Default: 1 (i.e. 0.01 if scale=2).
    let amountTolerance = Number(elTol?.value);

    if (!Number.isFinite(amountTolerance)) {
      // Convert visible tolerance to scaled integer if possible.
      if (Number.isFinite(visibleTol) && Number.isFinite(amountScale)) {
        amountTolerance = Math.max(0, Math.round(visibleTol * Math.pow(10, amountScale)));
      } else {
        amountTolerance = 1;
      }
    }

    const reportLimit = Number(elLimit?.value);

    const statusMap = new Map();
    const rawMap = (elMap?.value || "");
    ...
    return {
      amountScale: Number.isFinite(amountScale) ? amountScale : 2,
      amountTolerance: Number.isFinite(amountTolerance) ? amountTolerance : 1,
...
function renderResults(summary, downloads) {
  ...
  async function run() {
    ...
    if (errs.length) {
      ...
      showBanner(errs[0] || "Validation error", "warn", 6000);
      setProgress("Fix inputs and try again.", 0);
      return;
    }
    if (!window.Papa) {
...
state.lastDiagnostics = diag;
  ...
  // Events
  // If index.html uses <label for="filePicker"> (recommended), the browser opens the picker natively.
  // In that case we do NOT call addFileWithPicker() to avoid double handlers / blocked programmatic clicks.
  const _addBtn = $("#addFileBtn");
  if (_addBtn) {
    _addBtn.addEventListener("click", () => {
      const isLabel = (_addBtn.tagName === "LABEL") && (_addBtn.getAttribute("for") === "filePicker");
      if (!isLabel) addFileWithPicker();
    });
  }
  ...
  // Sync visible Tolerance control (index.html) → hidden amountTolerance (used by core engine).
  function syncToleranceToAmountTolerance() {
    const tol = document.querySelector("#tolerance");
    const amtTol = document.querySelector("#amountTolerance");
    if (!tol || !amtTol) return;
    const apply = () => {
      const v = String(tol.value ?? "").trim();
      // amountTolerance expects integer in scaled units; we convert:
      // amountTolerance_scaled = round(tolerance * 10^amountScale)
      const scaleEl = document.querySelector("#amountScale");
      const scale = scaleEl ? Number(scaleEl.value) : 2;
      const num = Number(v);
      if (!Number.isFinite(num)) return;
      const scaled = Math.max(0, Math.round(num * Math.pow(10, Number.isFinite(scale) ? scale : 2)));
      amtTol.value = String(scaled);
    };
    tol.addEventListener("change", apply);
    tol.addEventListener("input", apply);
    apply();
  }
  syncToleranceToAmountTolerance();
