...
const state = {
  ...
  exportSessions: new Map(), // pairKey|reportName -> {headers, chunks[]}
};
...
function onWorkerMessage(ev){
  ...
  if (msg.type === "EXPORT_META") {
    ...
    state.exportSessions.set(k, { headers: null, rows: [] });
  }
...
function renderRowsTable(pairKey, reportName, rows){
  ...
  if (!slice.length) return `<div class="muted small">No rows</div>`;
  ...
  const note = rows.length > cap ? `<div class="muted small">Showing first ${cap} rows. Use Export for full.</div>` : "";
  return `
    ${note}
    <div class="tableWrap">
      <table class="table">
