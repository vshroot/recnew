...
const SAFE_UI_CAP = 50000;     // max rows per report sent to UI
  ...
  return new Promise((resolve, reject) => {
    ...
    Papa.parse(file, {
      ...
      complete: () => {
        ...
        resolve({ counts, rowsTotal: rows, duplicatesRowsTotal: dupRows, duplicatesTxids: dupTxids });
      },
...
// 3.10: remove keyIndex Map to reduce memory. Use binary search on sorted uniqueKeys.
function findPosSorted(keys, key) {
  let lo = 0;
...
self.onmessage = async (ev) => {
  ...
  try {
    ...
    if (msg.type === 'RUN') {
      ...
      const csvDelimiter = (gs && gs.csvDelimiter) ? String(gs.csvDelimiter) : '';
      ...
      // base pass 1
      const baseCfg = Object.assign({}, msg.primary, (delim ? { delimiter: delim } : {}));
      ...
      for (let i = 0; i < otherCfgs.length; i++) {
        const c0 = otherCfgs[i];
        const c = Object.assign({}, c0, (delim ? { delimiter: delim } : {}));
        ...
        others.push({ cfg: c, counts: r.counts, rowsTotal: r.rowsTotal, duplicatesRowsTotal: r.duplicatesRowsTotal, duplicatesTxids: r.duplicatesTxids });
      }
    ...
    if (msg.type === 'EXPORT') {
      ...
      post('EXPORT_META', { pairKey, reportName });
      ...
      const pushRow = (row) => {
        ...
        if (chunk.length >= EXPORT_CHUNK_ROWS) {
          post('EXPORT_CHUNK', { headers, rows: chunk.splice(0, chunk.length) });
        }
      };
      ...
      if (chunk.length) post('EXPORT_CHUNK', { headers, rows: chunk });
      post('EXPORT_DONE', { pairKey, reportName });
      return;
    }
